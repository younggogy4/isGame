...
public class DoorManager
{
    ...
    public void Fill(GameData gameData, RoomManager roomManager)
    {
        ...
        door.Bounds = door.Bounds = new Rectangle((int)door.Position.X, (int)door.Position.Y, 200, 200);
        Doors.Add(door);*/
        var door1 = new DoorData();
        door1.Position = positionsDoor[1];
        door1.Bounds = door1.Bounds = new Rectangle((int)door1.Position.X, (int)door1.Position.Y, 400, 400);
         Doors.Add(door1);
    ...
    {
        if (gameData.EnemyManager.Enemies.Count <= 0)
        {
